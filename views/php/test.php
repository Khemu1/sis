...
{
  $data = Students::select(["userName", "name", "address", "level"], ["accountId" => 138])[0];
  var_dump($data);
  $info = "<div class=\"basic-info\">
  <h2>Student Information</h2>
  <div class=student-info>
    <p class=basic-info
    }} id=username>Username: {$data["userName"]} </p>
    <p class= info-item  class= info-item 
    } id=name>Name: {$data["name"]} </p>
    <p class= info-item 
    } id=address>Address: {$data["address"]} </p>
    <p class= info-item 
    } id=level>Level: {$data["level"]} </p>
  </div>
</div>";
  echo $info;
}
